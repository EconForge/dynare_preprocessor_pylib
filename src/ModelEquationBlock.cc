...
Epilogue::writeStaticEpilogueFile(const string& basename) const
{
  ...
  output << "function epilogue = epilogue_static(y, x, params)" << '\n'
         ...
         << "epilogue = NaN(" << symbol_table.epilogue_nbr() << ", 1);" << '\n';
  ...
  for (const auto& [symb_id, expr] : static_def_table)
    {
      ...
      if (expr->containsExternalFunction())
        expr->writeExternalFunctionOutput(output, ExprNodeOutputType::matlabStaticModel, {}, {},
                                          tef_terms);
      output << "epilogue(" << symbol_table.getTypeSpecificID(symb_id) + 1 << ") = ";
      expr->writeOutput(output, ExprNodeOutputType::matlabStaticModel, {}, {}, tef_terms);
      output << ";" << '\n';
    }
  output << "end" << '\n';
  output.close();
...
Epilogue::writeDynamicEpilogueFile(const string& basename) const
{
  ...
  output << "function ds = epilogue_dynamic(params, ds, steady_state, epilogue_steady_state)"
         << '\n'
         << "% function ds = epilogue_dynamic(params, ds, steady_state, epilogue_steady_state)"
         << '\n'
         << "% Epilogue file generated by Dynare preprocessor" << '\n'
         << '\n'
...
Epilogue::writeOutput(ostream& output) const
{
  if (dynamic_def_table.empty())
    {
      output << "M_.epilogue_var_list_ = {};" << '\n' << "M_.epilogue_varexo_list_ = {};" << '\n';
      return;
    }

  set<int> endogs, exogs;
  for (const auto& [symb_id, expr] : dynamic_def_table)
    {
  ...
  vector<string> symbol_list_endo;
  symbol_list_endo.reserve(endogs.size());
  for (auto symb_id : endogs)
    symbol_list_endo.push_back(symbol_table.getName(symb_id));
  SymbolList {move(symbol_list_endo)}.writeOutput("M_.epilogue_var_list_", output);

  vector<string> symbol_list_exo;
  symbol_list_exo.reserve(exogs.size());
  for (auto symb_id : exogs)
    symbol_list_exo.push_back(symbol_table.getName(symb_id));
  SymbolList {move(symbol_list_exo)}.writeOutput("M_.epilogue_varexo_list_", output);
}
