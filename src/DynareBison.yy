...
%token SKEWED_KALMAN_PRUNE_TOL SKEWED_KALMAN_RANK_DEFICIENCY_TRANSFORM SKEWED_KALMAN_MVNLOGCDF
%token LAPLACE LIK_INIT LINEAR LOAD_IDENT_FILES LOAD_MH_FILE LOAD_RESULTS_AFTER_LOAD_MH LOAD_PARAMS_AND_STEADY_STATE LOGLINEAR LOGDATA LYAPUNOV LINEAR_APPROXIMATION
...
%token MODEL_DIAGNOSTICS MODIFIEDHARMONICMEAN MOMENTS_VARENDO CONTEMPORANEOUS_CORRELATION DIFFUSE_FILTER ESTIMATE_INITIAL_STATES_ENDOGENOUS_PRIOR FREQUENTIST_SMOOTHER USE_PCT SUB_DRAWS TAPER_STEPS GEWEKE_INTERVAL RAFTERY_LEWIS_QRS RAFTERY_LEWIS_DIAGNOSTICS BROOKS_GELMAN_PLOTROWS MCMC_JUMPING_COVARIANCE MOMENT_CALIBRATION
...
%token OBSERVATION_TREND_BASE_PERIOD
...
%token SHOCKS HETEROSKEDASTIC_SHOCKS SHOCK_DECOMPOSITION SHOCK_GROUPS USE_SHOCK_GROUPS SIMUL ENDOGENOUS_TERMINAL_PERIOD
%token SMOOTHER SMOOTHER2HISTVAL SQUARE_ROOT_SOLVER STACK_SOLVE_ALGO STEADY_STATE_MODEL SOLVE_ALGO SOLVER_PERIODS ROBUST_LIN_SOLVE ALLOW_NONFINITE_VALUES
%token STDERR STEADY STOCH_SIMUL REGIMES REGIME REALTIME_SHOCK_DECOMPOSITION FORECAST_TYPE CONDITIONAL UNCONDITIONAL
...
%token SIMULATION_MULTIPLE SEED BOUNDED_SHOCK_SUPPORT ADDITIONAL_OPTIMIZER_STEPS SE_TOLX BURNIN
...
observation_trends : OBSERVATION_TRENDS ';' trend_list END ';' { driver.set_trends(); }
                   | OBSERVATION_TRENDS '(' OBSERVATION_TREND_BASE_PERIOD EQUAL integer_or_date ')' ';' trend_list END ';'
                     { driver.set_trends($5); }
                   ;
...
o_allow_nonfinite_values : ALLOW_NONFINITE_VALUES { driver.option_num("simul.allow_nonfinite_values", "true"); };
...
o_forecast_type : FORECAST_TYPE EQUAL UNCONDITIONAL
                  { driver.option_str("shock_decomp.forecast_type", "unconditional"); }
                | FORECAST_TYPE EQUAL CONDITIONAL
...
o_frequentist_smoother: FREQUENTIST_SMOOTHER { driver.option_num("frequentist_smoother", "true"); }
                      | FREQUENTIST_SMOOTHER EQUAL boolean { driver.option_num("frequentist_smoother", $3); };
...
o_analytic_derivation : ANALYTIC_DERIVATION { driver.option_num("analytic_derivation", "true"); }
                      | ANALYTIC_DERIVATION EQUAL boolean { driver.option_num("analytic_derivation", $3); }
                      ;
