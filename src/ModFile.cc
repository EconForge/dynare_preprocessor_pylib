...
void
ModFile::writePythonOutput(const string& basename, bool use_jax, bool use_numba) const
{
  filesystem::path python_dir = filesystem::path(basename) / "python";
  if (!filesystem::exists(python_dir))
    filesystem::create_directories(python_dir);

  writePythonDriverFile(basename, use_jax, use_numba);

  writePythonInitFile(python_dir);
  writePythonModelInterfaceFile(python_dir, use_jax, use_numba);
  writePythonTypeStubs(basename);

  if (dynamic_model.equation_number() > 0)
    {
      if (!no_static)
        {
          static_model.writePythonStaticFile(basename, use_jax, use_numba);
          static_model.writePythonStaticParamsDerivatives(basename, use_jax, use_numba);
        }
      dynamic_model.writePythonDynamicFile(basename, use_jax, use_numba);
      dynamic_model.writePythonDynamicParamsDerivatives(basename, use_jax, use_numba);
    }

  steady_state_model.writePythonSteadyStateFile(basename, use_jax, use_numba);

  cout << "Python output files generated successfully." << '\n';
}
...
void
ModFile::writePythonDriverFile(const string& basename, bool use_jax, bool use_numba) const
{
  filesystem::path python_dir = filesystem::path(basename) / "python";
  ostringstream output;

  output << R"(""")" << '\n'
         << "Dynare Model: " << basename << '\n'
         << "Generated automatically by Dynare preprocessor" << '\n'
         << R"(""")" << '\n'
         << '\n';

  output << DataTree::writePythonHelpersImports(use_jax, use_numba) << '\n';
...
void
ModFile::writePythonInitFile(const filesystem::path& python_dir) const
{
  ostringstream output;
  output << R"(""")" << '\n'
         << "Dynare Model Package" << '\n'
         << "Generated by Dynare Preprocessor" << '\n'
         << R"(""")" << '\n'
         << '\n'
         << "# Helper functions for derivatives" << '\n'
         << DataTree::writePythonHelpersDefinition() << '\n'
         << "# Import generated model modules" << '\n'
         << "from . import static" << '\n'
         << "from . import dynamic" << '\n'
         << '\n'
         << "# Import steady state module if it exists" << '\n'
         << "try:" << '\n'
         << "    from . import steadystate" << '\n'
         << "    __all__ = ['static', 'dynamic', 'steadystate']" << '\n'
         << "except ImportError:" << '\n'
...
void
ModFile::writePythonModelInterfaceFile(const filesystem::path& python_dir, bool use_jax,
                                       bool use_numba) const
{
  ostringstream output;

  output << R"(""")" << '\n'
         << "Model interface functions for numerical computation" << '\n'
         << R"(""")" << '\n'
         << '\n';

  output << DataTree::writePythonHelpersImports(use_jax, use_numba) << '\n';

  // Import the generated static and dynamic model files
  output << "from . import static as static_model" << '\n';
  output << "from . import dynamic as dynamic_model" << '\n' << '\n';

  output << DataTree::writePythonHelpersDecorator(use_jax, use_numba)
         << "def static_resid(y: np.ndarray, x: np.ndarray, params: np.ndarray) -> np.ndarray:"
         << '\n'
         << "    " << R"(""")" << '\n'
         << "    Compute static model residuals" << '\n'
         << "    " << '\n'
         << "    Parameters" << '\n'
         << "    ----------" << '\n'
         << "    y : np.ndarray" << '\n'
         ...
         << "        Residuals" << '\n'
         << "    " << R"(""")" << '\n'
         << "    return static_model.static_resid(y, x, params)" << '\n'
         << '\n';

  output << DataTree::writePythonHelpersDecorator(use_jax, use_numba)
         << "def dynamic_resid(y: np.ndarray, x: np.ndarray, params: np.ndarray, " << '\n'
         << "                  steady_state: np.ndarray, it_: int) -> np.ndarray:" << '\n'
         << "    " << R"(""")" << '\n'
         << "    Compute dynamic model residuals" << '\n'
         << "    " << '\n'
         << "    Parameters" << '\n'
         << "    ----------" << '\n'
         << "    y : np.ndarray" << '\n'
         << "        Endogenous variables (including leads and lags)" << '\n'
         << "    x : np.ndarray" << '\n'
         << "        Exogenous variables" << '\n'
         << "    params : np.ndarray" << '\n'
         << "        Parameters" << '\n'
         ...
         << "        Residuals" << '\n'
         << "    " << R"(""")" << '\n'
         << "    return dynamic_model.dynamic_resid(y, x, params, steady_state, it_)" << '\n';
...
void
ModFile::writePythonTypeStubs(const string& basename) const
{
  filesystem::path python_dir = filesystem::path(basename) / "python";
  ostringstream output;

  output << "# Type stubs for Dynare model" << '\n'
         << "import numpy as np" << '\n'
