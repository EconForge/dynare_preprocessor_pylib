                                                 ...
                                                 vector<string> target_eqtags_arg)
{
  if (isExistingTrendComponentModelName(name_arg))
    throw StatementException("trend_component_model",
                             "a trend component model already exists with the name " + name_arg);
...
TrendComponentModelTable::checkModelName(const string& name_arg) const
{
  if (!isExistingTrendComponentModelName(name_arg))
    throw StatementException("trend_component_model",
                             name_arg + " is not a recognized equation tag of a trend component model equation");
}
...
TrendComponentModelTable::writeOutput(const string& basename, ostream& output) const
{
  ...
  ar_ec_output << "function [AR, A0, A0star] = trend_component_ar_a0(model_name, params)" << '\n'
  ...
  for (const auto& name : names)
    {
      output << "M_.trend_component." << name << ".model_name = '" << name << "';" << '\n'
      ...
      ar_ec_output << "if strcmp(model_name, '" << name << "')" << '\n'
                   << "    % AR" << '\n'
  ...
  ar_ec_output << "error([model_name ' is not a valid trend_component_model name'])" << '\n'
               << "end" << '\n';
  ar_ec_output.close();
}
...
VarModelTable::addVarModel(string name_arg, bool structural_arg, vector<string> eqtags_arg)
{
  if (isExistingVarModelName(name_arg))
    throw StatementException("var_model", "a VAR model already exists with the name " + name_arg);
...
VarModelTable::writeOutput(const string& basename, ostream& output) const
{
  ...
  if (!ar_output.is_open())
    throw FileIOException(filename.string(), "Can't open file " + filename.string() + " for writing");
  ar_output << "function [ar, a0, constants] = varmatrices(model_name, params, reducedform)" << '\n'
            << "% File automatically generated by the Dynare preprocessor" << '\n'
            << '\n'
            << "if nargin<3" << '\n'
            << "    reducedform = false;" << '\n'
  ...
  for (const auto& name : names)
    {
      output << "M_.var." << name << ".model_name = '" << name << "';" << '\n'
      ...
      for (bool it : diff.at(name))
        output << boolalpha << it << " ";
      output << "];" << '\n'
             << "M_.var." << name << ".nonstationary = M_.var." << name << ".diff;" << '\n'
      ...
      ar_output << "if strcmp(model_name, '" << name << "')" << '\n'
      ...
      ar_output << "        if reducedform" << '\n'
                << "            for i=1:" << getMaxLag(name) << '\n'
                << "                ar(:,:,i) = a0\\ar(:,:,i);" << '\n'
                << "            end" << '\n'
                << "            if nargout<3" << '\n'
                << "                a0 = eye(" << lhs.size() << ");" << '\n'
  ...
  ar_output << "error('%s is not a valid var_model name', model_name)" << '\n';
  ar_output.close();
}
...
{
  if (!isExistingVarModelName(name_arg))
    throw StatementException("var_model", name_arg + " is not a recognized equation tag of a VAR model equation");
}
                                                 ...
                                                 expr_t discount_arg, int time_shift_arg)
{
  if (isExistingVarExpectationModelName(name_arg))
    throw StatementException("var_expectation_model",
                             "a var_expectation_model already exists with the name " + name_arg);
  ...
  for (const auto& name : names)
    {
      ...
      output << mstruct << ".auxiliary_model_name = '" << aux_model_name.at(name) << "';" << '\n'
             << mstruct << ".horizon = " << horizon.at(name) << ';' << '\n'
             << mstruct << ".time_shift = " << time_shift.at(name) << ';' << '\n';

      auto& vpc = vars_params_constants.at(name);
      if (!vpc.size())
        throw InternalCompilerException{"VarExpectationModelStatement::writeOutput: matchExpression() has not been called"};
      ...
      output << mstruct << ".expr.vars = [ " << vars_list.str() << " ];" << '\n'
             << mstruct << ".expr.params = [ " << params_list.str() << " ];" << '\n'
             << mstruct << ".expr.constants = [ " << constants_list.str() << " ];" << '\n';
  ...
  for (const auto& name : names)
    {
      ...
      if (var_model_table.isExistingVarModelName(aux_model_name[name]))
        ...
        throw StatementException("var_expectation_model",
                                 "var_expectation_model " + name + " refers to nonexistent auxiliary model " + aux_model_name[name]);
      ...
      try
        ...
        {
          throw StatementException("var_expectation_model",
                                   "expression in var_expectation_model " + name + " is not of the expected form: " + e.message);
        }
      ...
      if (var_expectation_subst_table.contains(name))
        throw StatementException("var_expectation_model",
                                 "model name '" + name + "' is used by several var_expectation_model statements");
                           ...
                           expr_t growth_arg, string auxname_arg, PacTargetKind kind_arg)
{
  if (isExistingPacModelName(name_arg))
    throw StatementException("pac_model", "a PAC model already exists with the name " + name_arg);
...
PacModelTable::checkPass(ModFileStructure& mod_file_struct)
{
    ...
    if (gv)
      {
        if (target_info.contains(name))
          throw StatementException("pac_model", "for PAC model '" + name
                                   + "', it is not possible to declare a 'growth' option in the 'pac_model' command when there is also a 'pac_target_info' block");
  ...
  for (auto& [name, auxn] : auxname)
    if (!auxn.empty() && target_info.contains(name))
      throw StatementException("pac_model", "for PAC model '" + name
                               + "', it is not possible to declare an 'auxname' option in the 'pac_model' command when there is also a 'pac_target_info' block");
    ...
    if (k != PacTargetKind::unspecified)
      {
        if (target_info.contains(name))
          throw StatementException("pac_model", "for PAC model '" + name
                                   + "', it is not possible to declare a 'kind' option in the 'pac_model' command when there is also a 'pac_target_info' block");
        if (aux_model_name[name].empty())
          throw StatementException("pac_model", "for PAC model '" + name
                                   + "', it is not possible to declare a 'kind' option in the 'pac_model' command since this is a MCE model");
      }
  ...
  for (const auto& [name, ti] : target_info)
    {
      ...
      if (!target)
        throw StatementException("pac_target_info", "the block 'pac_target_info(" + name + ")' is missing the 'target' statement");
      if (auxname_target_nonstationary.empty())
        throw StatementException("pac_target_info", "the block 'pac_target_info(" + name + ")' is missing the 'auxname_target_nonstationary' statement");
      ...
      for (auto& [component, growth_component, auxname, kind, coeff, growth_neutrality_param,
        ...
        {
          if (auxname.empty())
            throw StatementException("pac_target_info", "the block 'pac_target_info(" + name + ")' is missing the 'auxname' statement in some 'component'");
          if (kind == PacTargetKind::unspecified)
            throw StatementException("pac_target_info", "the block 'pac_target_info(" + name + ")' is missing the 'kind' statement in some 'component'");
          if (kind == PacTargetKind::ll && growth_component)
            throw StatementException("pac_target_info", "in the block 'pac_target_info(" + name + ")', a component of 'kind ll' (i.e. stationary) has a 'growth' option. This is not permitted.");
          if (kind == PacTargetKind::dd || kind == PacTargetKind::dl)
            nonstationary_nb++;
        }
      if (!nonstationary_nb)
        throw StatementException("pac_target_info", "the block 'pac_target_info(" + name + ")' must contain at least one nonstationary component (i.e. of 'kind' equal to either 'dd' or 'dl').");
    }
}
  ...
  for (const auto& name : names)
    {
      ...
      if (growth[name])
        try
          ...
          {
            throw StatementException("pac_model", "PAC growth must be a linear combination of variables");
          }
      ...
      if (target_info.contains(name))
        {
          ...
          if (neweqs.size() > 0)
            throw StatementException("pac_target_info", "the 'target' expression of 'pac_target_info("
                                     + name + ")' contains a variable with a unary operator that is not present in the model");
          target = target->substituteDiff(diff_nodes, diff_subst_table, neweqs);
          if (neweqs.size() > 0)
            throw StatementException("pac_target_info", "the 'target' expression of 'pac_target_info("
                                     + name + ")' contains a diff'd variable that is not present in the model");
          ...
          for (auto& [component, growth_component, auxname, kind, coeff, growth_neutrality_param,
                      h_indices, original_growth_component, growth_component_info] : components)
            {
              component = component->substituteUnaryOpNodes(unary_ops_nodes, unary_ops_subst_table,
                                                            neweqs);
              if (neweqs.size() > 0)
                throw StatementException("pac_target_info", "a 'component' expression of 'pac_target_info("
                                         + name + ")' contains a variable with a unary operator that is not present in the model");
          ...
          try
            ...
            {
              throw StatementException("pac_target_info", "there is no equation whose LHS is equal to the 'target' of 'pac_target_info("
                                       + name + ")'");
            }
          ...
          if (neweqs.size() > 0)
            throw StatementException("pac_target_info", "the equation defining the target of 'pac_target_info("
                                     + name + ")' contains a variable with a unary operator that is not present in the model");
          ...
          try
            ...
            {
              throw StatementException("pac_target_info", "the model equation defining the 'target' of 'pac_target_info("
                                       + name + ")' is not of the right form (should be a linear combination of endogenous variables)");
            }
          ...
          for (const auto& [var, coeff] : terms)
            if (auto it = ranges::find_if(
              ...
              throw StatementException("pac_target_info", "the model equation defining the 'target' of 'pac_target_info("
                                       ...
                                       + ") that is not declared as a 'component'");
          ...
          for (const auto& [component, growth_component, auxname, kind, coeff,
            ...
            if (!coeff)
              throw StatementException("pac_target_info", "a 'component' of 'pac_target_info(" + name
      ...
      if (trend_component_model_table.isExistingTrendComponentModelName(aux_model_name[name]))
        ...
        throw StatementException("pac_model", "aux_model_name not recognized as VAR model or Trend Component model");
      ...
      if (growth[name])
        {
          ...
          try
            ...
            {
              throw StatementException("pac_model", "The variable/parameter '" + param_name
                                       + "' conflicts with the auxiliary parameter that will be generated for the growth neutrality correction of the '"
                                       + name + "' PAC model. Please rename that parameter.");
            }
        }
        ...
        if (growth_symb_id)
          {
            ...
            if (symbol_table.getType(*growth_symb_id) == SymbolType::exogenous)
              {
                var_field = "exo_id";
                output << structname << "endo_id = 0;" << '\n';
              }
            else
              output << structname << "exo_id = 0;" << '\n';
            try
              {
                ...
                output << structname << var_field << " = "
                       << symbol_table.getTypeSpecificID(aux_symb_id) + 1 << ";" << '\n'
                       << structname << "lag = 0;" << '\n';
              }
            catch (...)
              {
                try
                ...
                catch (...)
                  {
                    ...
                    output << structname << var_field << " = "
                           << symbol_table.getTypeSpecificID(*growth_symb_id) + 1 << ";" << '\n'
                           << structname << "lag = " << growth_lag << ";" << '\n';
                  }
              }
          }
        else
  ...
  for (auto& [name, val] : equation_info)
    {
      ...
      if (optim_share_index)
        output << "M_.pac." << name << ".share_of_optimizing_agents_index = "
               << symbol_table.getTypeSpecificID(*optim_share_index) + 1 << ";" << '\n';
      ...
      for (auto& it : ec_params_and_vars.second)
        switch (symbol_table.getType(get<0>(it)))
          ...
          case SymbolType::exogenous:
            ...
            break;
          default:
            throw InternalCompilerException{"expecting endogenous or exogenous"};
          }
      ...
      for (auto& [pid, vid, vlag] : ar_params_and_vars)
        output << vlag << " ";
      output << "];" << '\n'
             << "M_.pac." << name << ".max_lag = " << pacEquationMaxLag(name) << ";" << '\n';
    ...
    for (int component_idx {1};
                ...
                h_indices, original_growth_component, growth_component_info] : get<2>(val))
      {
        ...
        output << fieldname << ".aux_id = " << symbol_table.getTypeSpecificID(auxname) + 1 << ";"
               ...
               << ";" << '\n'
               << fieldname << ".kind = '" << kindToString(kind) << "';" << '\n'
        ...
        if (growth_component)
          {
            output << fieldname << ".growth_neutrality_param_index = "
                   << symbol_table.getTypeSpecificID(growth_neutrality_param) + 1 << ";" << '\n'
    ...
    if (aux_model_name.at(name).empty())
      {
        ...
        output << pac_model_name << "z = NaN(" << get<2>(val).size() << ",1);" << '\n';
        for (int component_idx {1};
             auto& [component, growth_component, auxname, kind, coeff, growth_neutrality_param,
                    h_indices, original_growth_component, growth_component_info] : get<2>(val))
          {
            output << pac_model_name << "z(" << component_idx
                   << ") = " << symbol_table.getTypeSpecificID(auxname) + 1 << ";" << '\n';
            component_idx++;
          }
      }
}
...
PacModelTable::kindToString(PacTargetKind kind)
{
  switch (kind)
    ...
    case PacTargetKind::unspecified:
      throw InternalCompilerException{"kind should not be unspecified"};
...
PacModelTable::writeTargetCoefficientsFile(const string& basename) const
{
  ...
  if (!output.is_open())
    throw FileIOException(filename.string(), "Can't open file " + filename.string() + " for writing");
  output << "function coeffs = pac_target_coefficients(model_name, params)" << '\n';
  for (auto& [model_name, val] : target_info)
    {
      output << "  if strcmp(model_name, '" << model_name << "')" << '\n'
             << "    coeffs = NaN(" << get<2>(val).size() << ",1);" << '\n';
      for (int i {1};
           auto& [component, growth_component, auxname, kind, coeff, growth_neutrality_param,
                  h_indices, original_growth_component, growth_component_info] : get<2>(val))
        {
  ...
  output << "  error([ 'Unknown PAC model: ' model_name ])" << '\n' << "end" << '\n';
  output.close();
}
