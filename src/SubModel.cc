  ...
  if (isExistingTrendComponentModelName(name_arg))
    {
      ...
      err_msg << "Error: a trend component model already exists with the name " << name_arg << endl;
  ...
  if (!isExistingTrendComponentModelName(name_arg))
    {
      ...
      err_msg << name_arg << " is not a recognized equation tag of a trend component model equation"
           << endl;
      throw PreprocessorException(err_msg.str());
    }
}
  ...
  if (isExistingVarModelName(name_arg))
    {
      ...
      err_msg << "Error: a VAR model already exists with the name " << name_arg << endl;
      throw PreprocessorException(err_msg.str());
    }
  ...
  if (!isExistingVarModelName(name_arg))
    {
      ...
      err_msg << name_arg << " is not a recognized equation tag of a VAR model equation" << endl;
      throw PreprocessorException(err_msg.str());
    }
}
  ...
  if (isExistingVarExpectationModelName(name_arg))
    {
      ...
      err_msg << "Error: a var_expectation_model already exists with the name " << name_arg << endl;
      throw PreprocessorException(err_msg.str());
    }
      ...
      if (!vpc.size())
        {
          ...
          err_msg << "ERROR: VarExpectationModelStatement::writeOutput: matchExpression() has not "
                  "been called"
               << endl;
          throw PreprocessorException(err_msg.str());
        }
  ...
  for (const auto& name : names)
    {
      ...
      if (var_model_table.isExistingVarModelName(aux_model_name[name]))
      ...
      else
        {
          ...
          err_msg << "ERROR: var_expectation_model " << name
      ...
      try
      ...
      catch (ExprNode::MatchFailureException& e)
        {
          ...
          err_msg << "ERROR: expression in var_expectation_model " << name
  ...
  if (isExistingPacModelName(name_arg))
    {
      ...
      err_msg << "Error: a PAC model already exists with the name " << name_arg << endl;
      throw PreprocessorException(err_msg.str());
    }
        ...
        if (target_info.contains(name))
          {
            ...
            err_msg << "ERROR: for PAC model '" << name
      ...
      if (!target)
        {
          ...
          err_msg << "ERROR: the block 'pac_target_info(" << name
          ...
          if (kind == PacTargetKind::ll && growth_component)
            {
              ...
              err_msg << "ERROR: in the block 'pac_target_info(" << name
  ...
  for (const auto& name : names)
    {
      ...
      if (growth[name])
        try
        ...
        catch (ExprNode::MatchFailureException& e)
          {
            ...
            err_msg << "ERROR: PAC growth must be a linear combination of variables" << endl;
            throw PreprocessorException(err_msg.str());
          }
      ...
      if (target_info.contains(name))
        {
          ...
          if (neweqs.size() > 0)
            {
              ...
              err_msg << "ERROR: the 'target' expression of 'pac_target_info(" << name
              ...
              if (neweqs.size() > 0)
                {
                  ...
                  err_msg << "ERROR: a 'component' expression of 'pac_target_info(" << name
          ...
          try
          ...
          catch (ExprNode::MatchFailureException)
            {
              ...
              err_msg << "ERROR: there is no equation whose LHS is equal to the 'target' of "
                      "'pac_target_info("
                   << name << ")'" << endl;
              throw PreprocessorException(err_msg.str());
            }
          ...
          if (neweqs.size() > 0)
            {
              ...
              err_msg << "ERROR: the equation defining the target of 'pac_target_info(" << name
                   << ")' contains a diff'd variable that is not present in the model" << endl;
              throw PreprocessorException(err_msg.str());
            }
          ...
          try
          ...
          catch (ExprNode::MatchFailureException)
            {
              ...
              err_msg << "ERROR: the model equation defining the 'target' of 'pac_target_info(" << name
          ...
          for (auto [var, coeff] : terms)
            if (auto it = ranges::find_if(
            ...
            else
              {
                ...
                err_msg << "ERROR: the model equation defining the 'target' of 'pac_target_info("
            ...
            if (!coeff)
              {
                ...
                err_msg << "ERROR: a 'component' of 'pac_target_info(" << name
      ...
      if (trend_component_model_table.isExistingTrendComponentModelName(aux_model_name[name]))
      ...
      else
        {
          ...
          err_msg << "Error: aux_model_name not recognized as VAR model or Trend Component model"
               << endl;
          throw PreprocessorException(err_msg.str());
        }
      ...
      for (auto& it : ec_params_and_vars.second)
        switch (symbol_table.getType(get<0>(it)))
          ...
          default:
            ...
            err_msg << "expecting endogenous or exogenous" << endl;
            throw PreprocessorException(err_msg.str());
          }
...
PacModelTable::kindToString(PacTargetKind kind)
{
  switch (kind)
    ...
    case PacTargetKind::unspecified:
      ...
      err_msg << "Internal error: kind should not be unspecified" << endl;
      throw PreprocessorException(err_msg.str());
