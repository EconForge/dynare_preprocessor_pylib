                                                 ...
                                                 vector<string> target_eqtags_arg)
{
  if (isExistingTrendComponentModelName(name_arg))
    {
      cerr << "Error: a trend component model already exists with the name " << name_arg << '\n';
      exit(EXIT_FAILURE);
    }
...
VarModelTable::addVarModel(string name_arg, bool structural_arg, vector<string> eqtags_arg)
{
  if (isExistingVarModelName(name_arg))
    {
      cerr << "Error: a VAR model already exists with the name " << name_arg << '\n';
      exit(EXIT_FAILURE);
    }
...
VarModelTable::checkModelName(const string& name_arg) const
{
  if (!isExistingVarModelName(name_arg))
    {
      cerr << name_arg << " is not a recognized equation tag of a VAR model equation" << '\n';
      exit(EXIT_FAILURE);
    }
}
                                                 ...
                                                 expr_t discount_arg, int time_shift_arg)
{
  if (isExistingVarExpectationModelName(name_arg))
    {
      cerr << "Error: a var_expectation_model already exists with the name " << name_arg << '\n';
      exit(EXIT_FAILURE);
    }
      ...
      if (var_model_table.isExistingVarModelName(aux_model_name[name]))
      ...
      else
        {
          cerr << "ERROR: var_expectation_model " << name
               << " refers to nonexistent auxiliary model " << aux_model_name[name] << '\n';
          exit(EXIT_FAILURE);
        }
      ...
      try
      ...
      catch (ExprNode::MatchFailureException& e)
        {
          cerr << "ERROR: expression in var_expectation_model " << name
               << " is not of the expected form: " << e.message << '\n';
          exit(EXIT_FAILURE);
        }
      ...
      if (var_expectation_subst_table.contains(name))
        {
          cerr << "ERROR: model name '" << name
               << "' is used by several var_expectation_model statements" << '\n';
          exit(EXIT_FAILURE);
        }
                           ...
                           expr_t growth_arg, string auxname_arg, PacTargetKind kind_arg)
{
  if (isExistingPacModelName(name_arg))
    {
      cerr << "Error: a PAC model already exists with the name " << name_arg << '\n';
      exit(EXIT_FAILURE);
    }
      ...
      if (!target)
        {
          cerr << "ERROR: the block 'pac_target_info(" << name
               << ")' is missing the 'target' statement" << '\n';
          exit(EXIT_FAILURE);
        }
      if (auxname_target_nonstationary.empty())
        {
          cerr << "ERROR: the block 'pac_target_info(" << name
               << ")' is missing the 'auxname_target_nonstationary' statement" << '\n';
          exit(EXIT_FAILURE);
        }
      ...
      for (auto& [component, growth_component, auxname, kind, coeff, growth_neutrality_param,
                  h_indices, original_growth_component, growth_component_info] : components)
        {
          if (auxname.empty())
            {
              cerr << "ERROR: the block 'pac_target_info(" << name
                   << ")' is missing the 'auxname' statement in some 'component'" << '\n';
              exit(EXIT_FAILURE);
            }
          if (kind == PacTargetKind::unspecified)
            {
              cerr << "ERROR: the block 'pac_target_info(" << name
                   << ")' is missing the 'kind' statement in some 'component'" << '\n';
              exit(EXIT_FAILURE);
            }
  ...
  for (const auto& name : names)
    {
      ...
      if (growth[name])
        try
          ...
          {
            cerr << "ERROR: PAC growth must be a linear combination of variables" << '\n';
            exit(EXIT_FAILURE);
          }
          ...
          if (neweqs.size() > 0)
            {
              cerr << "ERROR: the 'target' expression of 'pac_target_info(" << name
                   << ")' contains a diff'd variable that is not present in the model" << '\n';
              exit(EXIT_FAILURE);
            }
      ...
      for (auto& it : ec_params_and_vars.second)
        switch (symbol_table.getType(get<0>(it)))
          ...
          case SymbolType::exogenous:
            ...
            break;
          default:
            cerr << "expecting endogenous or exogenous" << '\n';
            exit(EXIT_FAILURE);
          }
...
PacModelTable::kindToString(PacTargetKind kind)
{
  switch (kind)
    ...
    case PacTargetKind::unspecified:
      cerr << "Internal error: kind should not be unspecified" << '\n';
      exit(EXIT_FAILURE);
